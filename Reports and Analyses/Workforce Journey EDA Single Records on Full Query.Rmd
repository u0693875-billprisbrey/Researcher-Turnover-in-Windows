...
# On 8.18.2025, I am updating this report with a flow chart showing the filters and better logic defining the no concurrent jobs population.

# As of 7.10.2025, this pulls yet another new query.  The view is populated by a new query that is populated with the full historical records available of everyone who has activity after Jan 1, 2010.

# As of 7.8.2025, this pulls the new query.  This shifts the age younger, as you'd expect, and adds some 424 rows for some reason that might explain itself, or is irrelevant, or might be worth investigating and explaining.

# To do:
# - Include descriptive graphics of the new fields (job codes and descriptions)
# - Update the entry/exits graphic to completely match how I'm defining them
# - In a different report, calculate the headcount
# - I should add a dummy "entry" variable for people presumably entering before the min date (Jan 2, 2010.)  How am I going to double-check that? 
# - This query isn't exactly what I want.  It pulls all records after the defined date, but I want all the records for people who are -active- after the defined date.  Would it be so hard to create this query?  As it is, I almost want to identify all the EMPLIDs who have too many exits --- and pull their full history.  (Or maybe I just do this once, and if it's close enough then it's good enough ? )
# - I can remove the example tables.  They really aren't that useful.  If I wanted to populate, maybe I can just list EMPLIDs?  But why?
# - I could improve the titles and formatting of all of these tables (so many tables!  But they are useful reference look-ups for me.)
# - Write up how the "too many entries" might be an entry before the start date
...
# I already set a filter to the query to 6/30/2025
# Given that it's the week of maximum activity, I should just set it to Aug 1.
...
# Create "journeyPopulation" for flow chart;
# use this to identify employees with current and non-concurrent jobs
...
# PI (PI, not_PI)
# hcj (hire_conc, not_hire_conc)
# singleRD (single_rcd, mult_rcd)
# exclusiveJob (exclusive, concurrent)
# workbreak (wb, no_wb)
# leave (leave, no_leave)

# TRUE/FALSE
journeyPopulation$PI <- journeyPopulation$EMPLID %in% prepData$PROPOSAL_PI_EMPLID
journeyPopulation$hcj <- grepl("HCJ|RCJ", journeyPopulation$ACTION_REASON)
journeyPopulation$singleRCD <- journeyPopulation$EMPL_RCD == 1
journeyPopulation$exclusiveJob <- !journeyPopulation$hcj & journeyPopulation$singleRCD
journeyPopulation$workbreak <- grepl("SWB|RWB", journeyPopulation$ACTION)
journeyPopulation$leave <- grepl("RFL|LOA|LTO|PLA", journeyPopulation$ACTION)
...
# No concurrent jobs
journeyExclusiveFilter <- journeyData$EMPLID %in%  journeyPopulation_prosaic$EMPLID[journeyPopulation_prosaic$exclusiveJob == "exclusive"]

# PI's with no concurrent jobs
piexclusiveFilter <- journeyData$EMPLID %in%  journeyPopulation$EMPLID[journeyPopulation$exclusiveJob & 
                           journeyPopulation$PI]
  
journeyData <- journeyData[journeyExclusiveFilter,]

```

```{r eval=FALSE}  

## ABANDONED LOGIC; NOW USE "JOURNEY POPULATION"
...
journeyExclusiveFilter <- journeyData$EMPLID %in% singleRCD & journeyData$EMPL_RCD == 0 & (!journeyData$ACTION_REASON %in% "HCJ")
...
The "workforce journey" data is examined. The records are queried to identify all employees who have activity after 1 Jan 2010 (`r length(unique(jData$EMPLID)) |> format(big.mark = ",")` employees), and then their entire work history is pulled.  Their earliest activity date is `r as.Date(min(jData$EFFDT))` and their maximum activity date is cut off at `r as.Date(max(jData$EFFDT))`.  Then, employees who have had multiple concurrent jobs after January 2010 are identified and removed, and the population of remaining employees (called "exclusive") are described in this report.

For this population, some `r nrow(journeyData) |> format(big.mark=",")` rows use `r length(unique(journeyData$ACTION))` distinct actions to describe the employment experience of `r length(unique(journeyData$EMPLID)) |> format(big.mark = ",")` employees.
...
Several checks for consistency are applied.  Some `r do.call(rbind, tooManyExits) |> (\(x){length(unique(x$EMPLID))})() |> format(big.mark = ",")` (`r do.call(rbind, tooManyExits) |> (\(x){length(unique(x$EMPLID))})() |> (\(x){x/length(unique(journeyData$EMPLID))})() |> (\(x){round(100*x,1)})()`%) employees fail a check for too many "exits", particularly for a leave of absence.  Although these employees bear further scrutiny, this is a much lower number than for prior iterations of the query.

This query will be used going forward.    

**SUMMARY:**   

`r nrow(jData) |> format(big.mark=",")` rows covering HR  activity for `r length(unique(jData$EMPLID)) |> format(big.mark = ",")` employees is queried.   

All employees had activity after a cut-off date of 1 January 2010.  

The population is separated into employees with "concurrent" jobs and employees with "exclusive" jobs according to two conditions: (1) having multiple employment records indicating multiple concurrent jobs or (2) having "HCJ" ("Hire Concurrent Job") or "RCJ" ("Rehire Concurrent Job") in their action reasons. `r table(journeyPopulation$exclusiveJob)["FALSE"] |> format(big.mark =",")` employees ( `r table(journeyPopulation$exclusiveJob) |> proportions() |> (\(x){x["FALSE"]})() |> (\(x){round(x*100,0)})()`%) have concurrent jobs, and `r table(journeyPopulation$exclusiveJob)["TRUE"] |> format(big.mark =",")` employees (`r table(journeyPopulation$exclusiveJob) |> proportions() |> (\(x){x["TRUE"]})() |> (\(x){round(x*100,0)})()`%) have exclusive jobs, or one job at a time. 
...
Termination is almost exclusively flagged as "voluntary", with only `r proportions(table(journeyData$VOLUNTARY_FLAG))["Involuntary"] |> (\(x){round(100*x,1)})()`% of the termination actions described as "involuntary."    
...
A check for internal consistency compares the number of boundary actions per type per employee.  There should never be more exits than entries, for example, and never more than one additional entry than exits.  A tabulation of entries and exits per employee shows a large number of employees (`r do.call(rbind, tooManyExits) |> (\(x){length(unique(x$EMPLID))})() |> (\(x){x/length(unique(journeyData$EMPLID))})() |> (\(x){round(100*x,1)})()`%) with too many exits.  However, this tabulation is much lower for this query than for previous query iterations.  It bears further examination.   
...
journeyPopulation$exclusiveJob <- factor(journeyPopulation$exclusiveJob, levels = c(TRUE,FALSE))

journeyPopulation$hcj <- factor(journeyPopulation$hcj, levels = c(TRUE,FALSE))

journeyPopulation |>
  as_fc(label = "Query population", bg_fill = "ghostwhite") |>
    fc_split(exclusiveJob, label = c("Concurrent jobs", "No concurrent jobs"), bg_fill = "honeydew") |>
  fc_split(hcj, label = c("HCJ|RCJ as reason", "No such reasons"), bg_fill = "beige") |>
  fc_split(singleRCD, label = c("Multiple records", "Single record"), bg_fill = "whitesmoke") |>
#  fc_split(exclusiveJob, label = c("No concurrent jobs", "Concurrent jobs"), bg_fill = "ivory") |> 
  fc_draw()
...
journeyPopulation |>
  as_fc(label = "Query population", bg_fill = "ghostwhite") |>
    fc_split(exclusiveJob, label = c("Concurrent jobs", "No concurrent jobs"), bg_fill = "honeydew") |>
  fc_split(hcj, label = c("HCJ|RCJ as reason", "No such reasons"), bg_fill = "beige", sel_group = FALSE) |>
  fc_split(singleRCD, label = c("Multiple records", "Single Record"), bg_fill = "whitesmoke", , sel_group = c(TRUE)) |>
  fc_draw()


journeyPopulation |>
  as_fc(label = "Query population", bg_fill = "oldlace") |>
    fc_split(exclusiveJob, label = c("No concurrent jobs", "Concurrent jobs"), bg_fill = "linen") |>
  fc_split(hcj, label = c("Dog", "Cat")) |>
  fc_split(singleRCD, label = c("Record", "Man")) |>
#  fc_split(exclusiveJob, label = c("No concurrent jobs", "Concurrent jobs"), bg_fill = "ivory") |> 
  fc_draw()
...
journeyPopulation |>
  ...
  fc_split(exclusiveJob, label = c("Concurrent jobs", "No concurrent jobs"), bg_fill = "ivory") |> 
  fc_split(workbreak, label = c("No work break", "Work break"), bg_fill = "mintcream") |>
  fc_split(leave, label = c("No leave", "leave"), bg_fill = "plum1") |>
  fc_draw()

# To get the behavior I want, I'm going to need the columns to have different names (not all "TRUE/FALSE")

# I've stopped caring though

# Ok, let's get this to work CORRECTLY now
...
popChart <- journeyPopulation_prosaic |>
    as_fc(label = "Query population", bg_fill = "ghostwhite") |>
  fc_split(exclusiveJob, label = c("Concurrent", "Exclusive"), bg_fill = "honeydew", offset = -0.05) |> 
    fc_split(hcj, label = c("HCJ|RCJ as reason", "No such reasons"), bg_fill = "beige", offset = 0.025) |> 
  fc_split(singleRCD, label = c("Multiple records", "Single Record"), bg_fill = "whitesmoke", offset = 0.05) 

fc_draw(popChart, title = "Population of interest defined")
...
grid.text( "This report\ncovers employees\nwith exclusive jobs",
  x = mean(c(popChart$fc$x[3], 1) ),
  y = mean(c(popChart$fc$y[1], popChart$fc$y[9] )) ,
  just = c("center","top"),
  gp = gpar(col = "gray10", fontsize = 9, fontface = "italic")
)

# ALMOST ...
# THERE ...

# It would be nice to shift everything to the right
# It would be nice to have titles for each level
# And honestly I need to double-check this

# lunch break, though

# looks like I want the following
...
popChart <- journeyPopulation_prosaic |>
    as_fc(label = "Query population", bg_fill = "ghostwhite") |>
  fc_split(exclusiveJob, label = c("Concurrent jobs", "Exclusive jobs"), bg_fill = "honeydew", offset = -0.05) |> 
    fc_split(hcj, label = c("HCJ|RCJ as reason", "No such reasons"), bg_fill = "beige", offset = 0.025) |> 
  fc_split(singleRCD, label = c("Multiple records", "Single record"), bg_fill = "whitesmoke", offset = 0.05) 
...
grid.text( "This report\ncovers employees\nwith exclusive jobs",
  x = popChart$fc$x[3] + 0.67*diff(c(popChart$fc$x[3], 1) ),
  y = mean(c(popChart$fc$y[1], popChart$fc$y[9] )) ,
  just = c("center","top"),
  gp = gpar(col = "gray10", fontsize = 9, fontface = "italic")
)
...
incomingPar <- par(mar = c(4,13,3,1), bg = "ivory", fg = "gray10"  )

theBar <- barplot(actionFrame[order(actionFrame$count, decreasing = FALSE) ,"count"],
        col = "thistle",
        horiz = TRUE,
        las =2)
axis(side = 2, at = theBar, labels = actionFrame[order(actionFrame$count, decreasing = FALSE) ,"ACTION"], las = 2, line = -3, tick = FALSE)

axis(side = 2, at = theBar, labels = actionFrame[order(actionFrame$count, decreasing = FALSE) ,"ACTION_DESCR"], las = 2, line = 1)
...
# Reason graphic
# turn this horizontal? # DONE

incomingPar <- par(mar = c(5,13,3,1), bg = "ivory", fg = "gray10"  )
...
incomingPar <- par(mar = c(5,17,3,1), bg = "ivory", fg = "gray10"  )

plotFrame <- journeyData$JOB_TITLE |> 
  table() |>
  (\(x){data.frame(job = names(x), count = as.numeric(x))})() |>
...
axis(side = 2, at = theBar, labels = rev(plotFrame[,"job"]), las = 2, line = 1)

mtext(side = 3, font =2, cex = 1.3, "Top twenty jobs by row count", line = 1.319)
...
# This just isn't a very interesting graphic
...
actionCountPerEmplid <- aggregate(count ~ EMPLID + ACTION, 
          data = transform(journeyData, count = 1), 
          FUN = sum)

actionCountPerEmplid[actionCountPerEmplid$ACTION == "TER","count"] |> hist()
...
incomingPar <- par(mar = c(5,17,4,1), bg = "ivory", fg = "gray10"  )
...
mtext(side = 3, font =2, cex = 1.1, "Top twenty\ntermination reasons\nby count", line = .681)
...
explainHeadCount(rect_text_args=list(cex = 2.3))
...
# Although interesting, for the purposes of this report I don't need to see actual examples
