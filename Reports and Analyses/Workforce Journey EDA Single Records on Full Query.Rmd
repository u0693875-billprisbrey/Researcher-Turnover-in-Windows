...
journeyPopulation_prosaic$singleJob <- ifelse(journeyPopulation_prosaic$hcj == "not_hcj" & journeyPopulation_prosaic$singleRCD == "single_rcd", "exclusive", "concurrent")
...
# singleJob (exclusive, concurrent)
# workbreak (wb, no_wb)
...
# No concurrent jobs
journeySingleFilter <- journeyData$EMPLID %in%  journeyPopulation$EMPLID[journeyPopulation$singleJob]

# PI's with no concurrent jobs
piSingleFilter <- journeyData$EMPLID %in%  journeyPopulation$EMPLID[journeyPopulation$singleJob & 
...
journeyPopulation$singleJob <- factor(journeyPopulation$singleJob, levels = c(TRUE,FALSE))

journeyPopulation$hcj <- factor(journeyPopulation$hcj, levels = c(TRUE,FALSE))

journeyPopulation |>
  as_fc(label = "Query population", bg_fill = "ghostwhite") |>
    fc_split(singleJob, label = c("Concurrent jobs", "No concurrent jobs"), bg_fill = "honeydew") |>
...
#  fc_split(singleJob, label = c("No concurrent jobs", "Concurrent jobs"), bg_fill = "ivory") |> 
  fc_draw()
...
journeyPopulation |>
  as_fc(label = "Query population", bg_fill = "oldlace") |>
    fc_split(singleJob, label = c("No concurrent jobs", "Concurrent jobs"), bg_fill = "linen") |>
...
popChart <- journeyPopulation_prosaic |>
    as_fc(label = "Query population", bg_fill = "ghostwhite") |>
  fc_split(singleJob, label = c("Concurrent", "Exclusive"), bg_fill = "honeydew", offset = -0.05) |> 
