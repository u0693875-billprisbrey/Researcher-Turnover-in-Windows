...
**PURPOSE:**  The purpose of this report is to describe the "workforce journey" data for employees according to several filters:  First, that they have had some activity after Jan 1, 2010.  Second, that they only ever had one job, no concurrent jobs, and no multiple values for the "employee record" field.

**OBJECTIVES:**   

  1.  Describe the filters applied to the query with a flowchart.
  2.  Describe the "Workforce Journey" data for employees who had some activity after the cut-off date of 1 Jan 2010 and never had a concurrent job after the cut-off date of 1 Jan 2010.    
  3.  Define "boundary" actions and check for internal consistency.   

**EXECUTIVE SUMMARY:**

The "workforce journey" data is examined. The records are queried to identify all employees who have activity after 1 Jan 2010, and then their entire work history is pulled.  The earliest date is `r as.Date(min(journeyData$EFFDT))` and the maximum date is `r as.Date(max(journeyData$EFFDT))`.  It is filtered according to the condition of no concurrent jobs.  (More specifically, that the employee only ever had a EMPL_RCD value of zero and never had an action reason of "HCJ" ("Hire Concurrent Job") after the cut-off date.)  

In this data set, some `r nrow(journeyData) |> format(big.mark=",")` rows use `r length(unique(journeyData$ACTION))` distinct actions to describe the employment experience of `r length(unique(journeyData$EMPLID)) |> format(big.mark = ",")` employees.
...
`r nrow(jData) |> format(big.mark=",")` rows covering dates between `r min(journeyData$EFFDT)` and `r max(journeyData$EFFDT)` are queried.
