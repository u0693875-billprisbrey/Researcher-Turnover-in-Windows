...
However, of the target population of `r length(unique(prepData$PROPOSAL_PI_EMPLID)) |> format(big.mark = ",") ` PI's, some `r missingPercentage` have no matches.   These missing matches are not random, scoring a Kappa value of 0.29 when predicted with a machine learning model.  The missing PI's tend to be older, sparser researchers outside of the main researching institutions at the U. 
...
In conclusion, the data set is unattractive in determining an "academic age" per PI because it is too incomplete, too biased in missing values, and too inconsistent.
