...
# where daa.first_pub_yr <= 2025 AND daa.last_pub_yr <= 2025
...
As of report publication on 20 June 2025, `r length(unique(ageData$EMPLID)) |> format(big.mark = ",")` PI's have potential matches with `r length(unique(ageData$DIM_ID)) |> format(big.mark = ",")` researchers identified in Dimensions with varying counts of matches and varying levels of confidence.  

However, of the target population of `r length(unique(prepData$PROPOSAL_PI_EMPLID)) |> format(big.mark = ",") ` PI's, some `r missingPercentage` have no matches.   Further complicating an analysis, these missing matches are not random.  A predictive machine learning model scored a Kappa value of 0.29 where a Kappa value of less than 0.1 would indicate randomness.  Instead, the PI's without matches have less activity, older activity, and are outside of the main research institutions at the U.
...
The combined data set attempts to align `r length(unique(ageData$EMPLID)) |> format(big.mark = ",")` internal employee identification numbers with `r length(unique(ageData$DIM_ID)) |> format(big.mark = ",")` external researcher identification numbers and provides a score of confidence in the alignment.  It aligns `r length(unique(ageData$EMPLID[ageData$EMPLID %in% nonDupes & ageData$CONFIDENCE == 1])) |> format(big.mark = ",")` (`r  (length(unique(ageData$EMPLID[ageData$EMPLID %in% nonDupes & ageData$CONFIDENCE == 1])) / length(unique(ageData$EMPLID))) |> (\(x){round(x*100,0)})() `%) internal EMPLIDs with a one-to-one match at the highest confidence score.  The remaining `r length(unique(ageData$EMPLID[!(ageData$EMPLID %in% nonDupes) | ageData$CONFIDENCE != 1])) |> format(big.mark = ",")` EMPLID's are aligned with multiple potential external ID's at reduced confidence levels.  

An inconsistency is discovered, where `r length(unique(ageData$EMPLID[ageData$EMPLID %in% names(theDupes) & ageData$CONFIDENCE == 1]))` EMPLID's are aligned with multiple external ID's with a confidence level of 1.

The target population of `r length(unique(prepData$PROPOSAL_PI_EMPLID)) |> format(big.mark=",")` PI's is identified in previous research as principal investigators submitting a proposal between FY2013 and FY2023. Of this target population, `r missingPercentage` do not have matches in the Dimensions data; `r multiplePercentage` have multiple matches; and `r singlePercentage` have a single match of various confidence levels.
...
hist(log(ageData$TOTAL_PUBLICATIONS), col = "darkolivegreen3",
     ylab = "Count of PI's",
     xlab = "",
     main = "Total publications per PI (log)")

hist(log(ageData$TOTAL_GRANTS), col = "orchid",
     ylab = "Count of PI's",
     xlab = "",
     main = "Total grants per PI (log)")
...
Extreme gradient boosting was used with default settings to detect whether a PI match was "missing" or "present" in Dimensions data using `r ncol(fullPI) - 2` variables (`r sapply(fullPI, class) |> table() |> (\(x){ x[names(x) == "numeric"]})()` numeric and `r sapply(fullPI, class) |> table() |> (\(x){ x[names(x) == "character"]})()` categories.) 
