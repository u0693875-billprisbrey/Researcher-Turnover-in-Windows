...
actionReasonFrame$boundary_pair <- NA
...
actionReasonFrame$boundary_pair[primeEntryFilter|primeExitFilter] <- "primary"
...
actionReasonFrame$boundary_pair[breakEntryFilter|breakExitFilter] <- "break"
...
actionReasonFrame$boundary_pair[leaveEntryFilter|leaveExitFilter] <- "leave"

# now I gotta merge this back to the journey data

journeyData <- merge(journeyData, unique(actionReasonFrame[,c("ACTION","boundary","boundary_pair")]), by = "ACTION")
...
boundaryFrame <- as.data.frame(table(journeyData[,c("EMPLID","boundary_pair","boundary")]))

boundaryList <- lapply(levels(boundaryFrame$boundary_pair), function(x){ 
  
  aggregate(Freq~EMPLID + boundary, data = boundaryFrame[boundaryFrame$boundary_pair == x,], sum) |>
    (\(y){
      reshape(y,
              idvar = "EMPLID",
              timevar = "boundary",
...
names(boundaryList) <- levels(boundaryFrame$boundary_pair)
...
unique(actionReasonFrame[,c("ACTION", "ACTION_DESCR","boundary", "boundary_pair")]) |>
  (\(x){x <- x[!is.na(x$boundary),] })() |>
  (\(x){x[order(x$boundary_pair),]  })() |>
  
  kbl(caption = "Boundary actions grouped", row.names=FALSE)
...
sapply(tooManyExits, nrow) |> kbl(caption = "Too many exits", row.names=FALSE)

tooManyExits[[1]][1:10,] |>
  kbl(caption = "More 'break' exits than entries", row.names=FALSE)

sapply(tooManyEntries, nrow) |> kbl(caption = "Too many entries", row.names=FALSE)
...
shobsho <- aggregate(boundary~EMPLID + boundary_pair, data = journeyData, table) |> as.matrix.data.frame()

# that's not it

shobble <- as.data.frame(table(journeyData[, c("EMPLID","boundary_pair","boundary")]))
...
ohbsho <- lapply(levels(shobble$boundary_pair), function(x) {aggregate(Freq ~ EMPLID, data = shobble[shobble$boundary_pair == x,], length(unique(x)) )})


shobble[shobble$boundary_pair == "primary",] |> head()


shaibo <- aggregate(Freq ~ EMPLID + boundary, data = shobble[shobble$boundary_pair == "primary",], sum) |>
  (\(x){x[order(x$EMPLID),]})()
...
shazam <- lapply(levels(shobble$boundary_pair), function(x){ 
  
  aggregate(Freq~EMPLID + boundary, data = shobble[shobble$boundary_pair == x,], sum) |>
    (\(y){
      reshape(y,
              idvar = "EMPLID",
              timevar = "boundary",
...
names(shazam) <- levels(shobble$boundary_pair)
