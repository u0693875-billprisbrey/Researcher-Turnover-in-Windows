...
###############################
## ACTIVE PUBLISHING PERIODS ##
###############################

# aggregate(PROPOSAL_UPLOAD_DATE ~ PROPOSAL_PI_EMPLID, data = prepData, FUN = function(x) {c(min = min(x), max= max(x))}) |> head()

minDate <- aggregate(PROPOSAL_UPLOAD_DATE ~ PROPOSAL_PI_EMPLID, data = prepData, FUN =  function(x) {as.Date(min(x))}) # |> head() # yes

maxDate <- aggregate(PROPOSAL_UPLOAD_DATE ~ PROPOSAL_PI_EMPLID, data = prepData, FUN = function(x) {as.Date(max(x))}) # |> head() # yes

# aggregate(PROPOSAL_UPLOAD_DATE ~ PROPOSAL_PI_EMPLID, data = prepData, FUN = function(x) {c(min = as.Date(min(x)), max= as.Date(max(x))) }) |> head() # no

activePublishing <- merge(minDate, maxDate, by = "PROPOSAL_PI_EMPLID")
names(activePublishing) <- c("PROPOSAL_PI_EMPLID", "PROPOSAL_UPLOAD_DATE.min", "PROPOSAL_UPLOAD_DATE.max")

activePublishing$active <- time_length(interval(activePublishing$PROPOSAL_UPLOAD_DATE.min, activePublishing$PROPOSAL_UPLOAD_DATE.max), unit = "year")

activePublishing$inactive_to_date <- time_length(interval(activePublishing$PROPOSAL_UPLOAD_DATE.max, ymd("2025-05-01")), unit = "year")
  
# let's merge on the hire/rehire/term dates

activePublishing <- merge(activePublishing, retData[,c("PI_EMPLID", "HIRE_DT", "REHIRE_DT", "TERMINATION_DT")],
...
# hire-to-publish
# re-hire-to-publish
# last publish to term
# active-yet-inactive (yrs since last publish for current headcount)
# productive period (first to last publication)


# I need to differentiate "active"-ly publishing and "current"-ly headcount
...
# And I can figure out the correct period to use by comparing the min publishing date and the max hire/rehire date that is less than that.
...
table(is.na(activePublishing$REHIRE_DT))

# FALSE  TRUE 
#   796  2141 

# so I'm not dealing with too many re-hires

condition <- activePublishing$REHIRE_DT >= activePublishing$PROPOSAL_UPLOAD_DATE.min
  
dim(activePublishing[condition,])

# let's look at all the possible combinations

condition1 <- activePublishing$REHIRE_DT >= activePublishing$PROPOSAL_UPLOAD_DATE.min # These are active publishers after their rehire date

condition2 <- activePublishing$PROPOSAL_UPLOAD_DATE.max <= activePublishing$REHIRE_DT # These are active publishers before their rehire date
  
  
# I want:
#  Were they publishing between their hire and re-hire date?  Only before their re-hire date?
#  Were they publishing only after their re-hire date?
#  Were they publishing before AND after their re-hire date?

before_rehire <- activePublishing$PROPOSAL_UPLOAD_DATE.max <= activePublishing$REHIRE_DT

after_rehire <- activePublishing$PROPOSAL_UPLOAD_DATE.min >= activePublishing$REHIRE_DT

before_and_after <- activePublishing$PROPOSAL_UPLOAD_DATE.min <= activePublishing$REHIRE_DT &  activePublishing$PROPOSAL_UPLOAD_DATE.max >= activePublishing$REHIRE_DT
...
# I'll commit to here, and then I'm going to go back and clean up some of my code and thinking.
...
# Chat was trying to give me the duration of active publishing periods

# thanks Chat!
# or maybe not this is becoming unworkable to debug.
# Gave up and worked it through on my own

agg <- aggregate(PROPOSAL_UPLOAD_DATE ~ PROPOSAL_PI_EMPLID, data = prepData, 
                 FUN = function(x) {
                   min_date <- as.Date(min(x), origin = "1970-01-01")
                   max_date <- as.Date(max(x), origin = "1970-01-01")
                   duration <- as.integer(max_date - min_date)
                   c(start = min_date, end = max_date, duration_days = duration)
                 })

# Convert to a clean data frame
activePublishing <- do.call(data.frame, agg)

# Set column names explicitly
names(activePublishing)[2:4] <- c("start_date", "end_date", "duration_days")

# View result
head(activePublishing)
