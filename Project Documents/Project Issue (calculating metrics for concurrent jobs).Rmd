...
Plan A:  Divide the various individuals into different regimes and develop custom algorithmic treatment to identify university- and job- level exits and entries.  

Plan B:  "Brute force fit" approximation: calculate the cumulative headcount over each individual's workforce journey.  Never allow the headcount to exceed one or zero, and extract the dates where it changes. 
...
*23 Oct 2025:* While developing custom algorithms per regime, Bill developed a "brute force" algorithm where the cumulative headcount is calculated per individual and constrained to values of one or zero.  Then the dates of change are extracted as university-level boundary events.  This algorithm should work for all individuals across all regimes.  However, in a presentation to HR (Kirsten Allen and Brian Gelsinger) the resulting headcount was too high.  Brian will provide snapshot data, or lists of employees (EMPLID's) active in January and July going back five or ten years.  Bill will focus on the Chemical Engineering department to improve his algorithm.  
